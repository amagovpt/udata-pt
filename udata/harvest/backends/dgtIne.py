...
class DGTINEBackend(BaseBackend):
    ...
    # INE catalog endpoint (JSON)
    CATALOG_URL = "https://www.ine.pt/ine/catalogo_hvd.jsp?opc=4&lang=PT"

    def inner_harvest(self):
        # Download do JSON via helper guardado do BaseBackend
        # (SSRF check + retry/timeout) em vez de subprocess curl + /tmp
        headers = {
            "Accept": "application/json",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
        }
        try:
            response = self.get(self.CATALOG_URL, headers=headers)
            response.raise_for_status()
            data = response.json()
        except Exception as e:
            self.logger.error(f"Error fetching/parsing INE JSON: {e}")
            return
