...
class INEHvdBackend(BaseBackend):
    ...
    def inner_harvest(self):
        ...
        # Fetch the catalog through the guarded helper (SSRF check + retry/timeout)
        req = self.get(self.source.url)
    ...
    def inner_process_dataset(self, item: HarvestItem):
        ...
        req = self.get(final_url, headers={"charset": "utf8"})
        ...
        target = None
        for propNode in properties:
            if propNode.hasAttribute("id") and str(propNode.getAttribute("id")) == str(
                item.remote_id
            ):
                target = propNode
                break
        ...
        if dates_node:
            dataset.extras["last_period_available"] = get_text(
                dates_node[0].getElementsByTagName("last_period_available")
            )
            dataset.extras["last_update_remote"] = get_text(
                dates_node[0].getElementsByTagName("last_update")
            )
